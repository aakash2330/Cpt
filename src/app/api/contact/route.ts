...
const ContactFormSchema = z
  ...
  .superRefine((data, ctx) => {
    if (!data.name?.trim() && !data.full_name?.trim()) {
      ctx.addIssue({
...
export async function POST(request: NextRequest) {
  try {
    ...
    const formType = data.form_type || "Contact Form";
    const name = data.name || data.full_name || "";
    const company = data.company || data.company_name;
    const textLines = [
      `Form Type: ${formType}`,
      `Name: ${name}`,
      formatOptional("Company", company),
      formatOptional("Email", data.email),
      formatOptional("Phone", data.phone_number),
    ...
    await transporter.sendMail({
      ...
      to: recipient,
      subject: `CPT Website Inquiry: ${formType}`,
      text: textLines.join("\n"),
      html: `
